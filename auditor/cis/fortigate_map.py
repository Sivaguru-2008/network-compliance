"""
CIS FortiGate 7.0.x → SecurityBaselineModel mapping.
...
This module loads the authoritative mapping from fortigate_map.json and
...
def load_and_validate_mappings(json_path: Path) -> Dict[str, RuleMapping]:
    ...
    if not json_path.is_file():
        raise FileNotFoundError(f"FortiGate mapping file not found at {json_path}")
...
MAP_JSON_PATH = Path(__file__).parent / "fortigate_map.json"
FORTIGATE_RULE_MAP: Dict[str, RuleMapping] = load_and_validate_mappings(MAP_JSON_PATH)
