"""Deterministic Zscaler ZPA (Zscaler Private Access) API configuration parser.

Validation Status: SYNTHETIC / CLOUD API FORMAT.
Zscaler ZPA is a 100% cloud-delivered Zero Trust Network Access (ZTNA) platform configured
via Cloud Admin Portal and REST API JSON (no on-box text running-config exists). This parser
processes JSON API responses and must not be categorized as on-premises production device grammar.
"""
...
@registry.register
class ZscalerZPAParser(VendorParser):
    """JSON-based parser for Zscaler ZPA API configuration responses."""
    ...
    def detect(cls, config_text: str) -> float:
        ...
        score = 0.0
        # ZPA-specific API response keys
        zpa_keys = {"adminSso", "accessPolicies", "logReceivers", "applicationSegments", "zpa_configuration"}
        matching_keys = zpa_keys.intersection(data.keys())
        if matching_keys:
            score += 0.8
            ...
            if observation.note == "Parser did not evaluate this field.":
                setattr(
                    ...
                    type(observation).unknown(
                        "Zscaler ZPA parser does not evaluate this field."
                    )
                )
    ...
    def _normalize_hostname(self, data: dict, baseline: SecurityBaselineModel) -> None:
        ...
        # ZPA configuration tenant identification
        if "zpa_configuration" in data and isinstance(data["zpa_configuration"], dict):
            tenant_name = data["zpa_configuration"].get("tenant")
            path = "/zpa_configuration/tenant"
        elif "tenant" in data:
        ...
        if tenant_name and path:
            ...
            baseline.hostname = Observation[str].unknown("Zscaler ZPA tenant identification is not present.")

    def _normalize_http_https(self, baseline: SecurityBaselineModel) -> None:
        # ZPA Admin Portal enforces HTTPS administration connections
        baseline.http_server_enabled = Observation[bool].absent(
            False, "Zscaler ZPA Admin Portal does not support HTTP cleartext administration."
        )
        baseline.https_server_enabled = Observation[bool].absent(
            True, "Zscaler ZPA Admin Portal is strictly HTTPS-only."
        )
    ...
    def _normalize_ssh_telnet(self, baseline: SecurityBaselineModel) -> None:
        ...
        baseline.telnet_enabled = Observation[bool].absent(
            False, "Telnet access is unsupported in Zscaler ZPA."
        )
        baseline.ssh_enabled = Observation[bool].absent(
            False, "SSH access is unsupported in Zscaler ZPA."
        )
        baseline.vty_transport_input = Observation[List[str]].absent(
            [], "No VTY transports are supported in Zscaler ZPA."
        )
    ...
    def _normalize_logging(self, data: dict, baseline: SecurityBaselineModel) -> None:
        # Check ZPA Log Receivers configuration status
        receiver_configured = False
        path = None

        if "logReceivers" in data:
            if isinstance(data["logReceivers"], list) and len(data["logReceivers"]) > 0:
                receiver_configured = True
                path = "/logReceivers"
            elif isinstance(data["logReceivers"], dict):
                receiver_configured = True
                path = "/logReceivers"

        if receiver_configured and path:
            raw, line, note = self._evidence(path, "configured")
            baseline.logging_enabled = Observation[bool].found(True, raw, line, note=note)
            baseline.logging_hosts = Observation[List[str]].found(["Zscaler ZPA Log Receiver"], raw, line, note=note)
        else:
            baseline.logging_enabled = Observation[bool].unknown("Zscaler ZPA log receiver status is unknown.")
            baseline.logging_hosts = Observation[List[str]].unknown("Zscaler ZPA log hosts are unknown.")
