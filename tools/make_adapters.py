...
class AdapterRegistry:
    def __init__(self):
        self._adapters = {
            'cisco_ios': CiscoIOSAdapter, 'cisco_asa': CiscoASAAdapter, 'juniper_junos': JuniperJunosAdapter,
            'arista_eos': AristaEOSAdapter, 'fortinet_fortios': FortiOSAdapter, 'paloalto_panos': PaloAltoPANOSAdapter,
            'huawei_vrp': HuaweiVRPAdapter, 'nokia_sros': NokiaSROSAdapter, 'mikrotik_routeros': MikroTikRouterOSAdapter,
            'f5_bigip_tmos': F5BIGIPAdapter, 'sonic': SONiCAdapter, 'netgate_pfsense': pfSenseAdapter,
            'checkpoint_gaia': CheckPointGaiaAdapter, 'sonicwall_sonicos': SonicWallAdapter,
            'stormshield_sns': StormshieldAdapter, 'ubiquiti_edgeos': UbiquitiAdapter, 'versa_versos': VersaAdapter,
            'watchguard_fireware': WatchGuardAdapter, 'hpe_aruba_aos_cx': HPEArubaAdapter,
            'extreme_exos': ExtremeEXOSAdapter, 'alcatel_aos': AlcatelAOSAdapter, 'a10_acos': A10ACOSAdapter,
            'sophos_sfos': SophosSFOSAdapter, 'ruckus_fastiron': RuckusFastIronAdapter,
            'sangfor_ngaf': SangforNGAFAdapter, 'hillstone_stoneos': HillstoneStoneOSAdapter,
            'barracuda_cloudgen': BarracudaCloudGenAdapter, 'cato_networks': CatoNetworksAdapter,
            'forcepoint_ngfw': ForcepointNGFWAdapter, 'zscaler_zia': ZscalerZIAAdapter, 'zscaler_zpa': ZscalerZPAAdapter,
        }
